...
| `sync_messages` | `folder`, `delta_token`, `limit` | What **changed** since the token, including `removed: true` entries. Use over a date window when edits/deletions matter — `receivedDateTime` never moves after delivery. Tokens expire → re-sync from scratch |
...
| Tool | Key parameters | Notes |
|------|---------------|-------|
| `create_event` | `subject`, `start_iso`, `end_iso`, `attendees`, `timezone`, `repeat`, `repeat_days`, `repeat_count`, `repeat_until`, `dry_run=True` | Graph emails invitations immediately, so dry-run is **on** by default. An offsetless `start_iso`/`end_iso` needs `timezone` (or `MSGRAPH_DEFAULT_TIMEZONE`) or the call is refused |
...
- **Recurrence** (`calendar.py:_recurrence`): `repeat` is a compact vocabulary (`daily`/`weekly`/`monthly`/`yearly`) over Graph's six pattern types; weekly needs `repeat_days`, monthly/yearly derive their day from `start_iso`. `list_events` uses `calendarView`, which **expands** a series into occurrences — plain `/events` returns only the master, so do not switch it. Deleting a `seriesMaster` destroys every occurrence; the dry-run says so.
- **`Prefer` must be merged, never assigned** (`graph.py:_merge_prefer`): it is a comma-separated list, and overwriting it silently drops `outlook.body-content-type="text"` on body fetches. Immutable ids live in `_headers()` so they ride continuation pages too — keying them off `params` would give page 1 and page 2 different id types.
...
- **Category filters go server-side; flag filters cannot** (`mail.py`): Exchange accepts `categories/any(c:c eq '…')` alongside `$orderby`, so `category` is pushed into the `$filter` on both `list_messages` and the bulk scan. Free-text category names must go through `_odata_string`, which doubles embedded quotes — they are the only caller-supplied value reaching a `$filter` without a parser or enum in front of it.
